...
function RecentsSection({ isHistoryEmpty, chatSessions }: RecentsSectionProps) {
  ...
  return (
    ...
    >
      <SidebarSection title="Verlauf">
        {isHistoryEmpty ? (
          <Text text01 className="px-3">
            Starten Sie einen Chat! Ihr Chat-Verlauf wird hier erscheinen.
          </Text>
        ) : (
          ...
          onConfirm={async (doNotShowAgain: boolean) => {
            ...
            if (chat && target != null) {
              try {
                ...
                showErrorNotification(
                  setPopup,
                  "Der Chat konnte nicht verschoben werden. Bitte versuchen Sie es erneut."
                );
              }
            }
          }}
