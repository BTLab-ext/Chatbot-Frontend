...
function FilePickerPopoverContents({
  ...
  const shouldShowMoreFilesButton = true;
    ...
    deleteUserFile(file.id)
      .then((result) => {
        if (!result.has_associations) {
          ...
          let message = "Datei kann nicht gelöscht werden. Sie ist beim";
          if (projects) {
            message += ` Projekt: ${projects}`;
          }
          if (projects && assistants) {
            message += " und ";
          }
          if (assistants) {
            message += `Antwortprofile: ${assistants}`;
          }
          message += ` eingebunden.`;
          setPopup({
            message: message,
      ...
      .catch((error) => {
        ...
        setPopup({
          message: "Datei konnte nicht gelöscht werden. Bitte versuchen Sie es erneut.",
          type: "error",
        });
        // Useful for debugging; safe in client components
        console.error("Datei konnte nicht gelöscht werden,", error);
      });
  };
        ...
        {showRecentFiles && (
          ...
          >
            <UserFilesModalContent
              ...
              description="Dateien hochladen oder aus Ihren zuletzt verwendeten Dateien auswählen."
              icon={SvgFiles}
              recentFiles={recentFilesSnapshot}
              onPickRecent={(file) => {
                onPickRecent && onPickRecent(file);
